(function () {
  function init() {
    ...
    document.querySelectorAll(".js-carousel-embla-basic").forEach((root) => {
      if (root.dataset.carouselEmblaBasicInit) return;
      root.dataset.carouselEmblaBasicInit = "true";

      const viewport = root.querySelector(".carousel-embla-basic__viewport");
      const container = root.querySelector(".carousel-embla-basic__container");
      if (!viewport || !container) return;

      const originalSlides = Array.from(
        container.querySelectorAll(".carousel-embla-basic__slide"),
      );
