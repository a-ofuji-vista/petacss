(function () {
  ...
  document.querySelectorAll(".js-modal-basic-scroll").forEach((root) => {
    const openButton = root.querySelector(".js-modal-basic-scroll-open");
    const dialog = root.querySelector(".js-modal-basic-scroll-dialog");
    if (!openButton || !dialog) return;

    instanceIndex += 1;
    const title = root.querySelector(".modal-basic-scroll__title");
    if (title) {
      const titleId = `modal-basic-scroll-title-${instanceIndex}`;
      title.id = titleId;
      dialog.setAttribute("aria-labelledby", titleId);
    }

    const visibleClass = "modal-basic-scroll__dialog--visible";
    let isClosing = false;
    let openRafId = 0;
    ...
    dialog
      .querySelectorAll(".js-modal-basic-scroll-close")
      .forEach((button) => {
