(function () {
  ...
  document.querySelectorAll(".js-modal-basic").forEach((root) => {
    const openButton = root.querySelector(".js-modal-basic-open");
    const dialog = root.querySelector(".js-modal-basic-dialog");
    if (!openButton || !dialog) return;

    instanceIndex += 1;
    const title = root.querySelector(".modal-basic__title");
    if (title) {
      const titleId = `modal-basic-title-${instanceIndex}`;
      title.id = titleId;
      dialog.setAttribute("aria-labelledby", titleId);
    }

    const visibleClass = "modal-basic__dialog--visible";
    let isClosing = false;
    let openRafId = 0;
    ...
    dialog.querySelectorAll(".js-modal-basic-close").forEach((button) => {
